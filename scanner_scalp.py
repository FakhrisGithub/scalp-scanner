...
def fetch_df(symbol: str, interval: str, limit: int = 200) -> pd.DataFrame:
...
def scalp_confluence(sig1m, sig3m, sig5m, sig15m, news_boost) -> tuple:
    sigs = [sig1m, sig3m, sig5m, sig15m]
    ...
    elif long_v == 2 and sig15m["bias"] == "LONG":
        bias = "LONG";  vote_sc = 12
    elif short_v == 2 and sig15m["bias"] == "SHORT":
    ...
    vol_ok  = sig5m["rel_vol"] >= 1.3 or sig15m["rel_vol"] >= 1.3
    ...
    detail = f"L:{long_v}/S:{short_v} Vol:{round(max(sig5m['rel_vol'], sig15m['rel_vol']),1)}x"
...
def analyze_scalp(symbol: str):
    try:
        df1m  = add_indicators(fetch_df(symbol, "1",  limit=150))
        df3m  = add_indicators(fetch_df(symbol, "3",  limit=150))
        df5m  = add_indicators(fetch_df(symbol, "5",  limit=150))
        df15m = add_indicators(fetch_df(symbol, "15", limit=150))

        sig1m  = scalp_signal_tf(df1m)
        sig3m  = scalp_signal_tf(df3m)
        ...
        final, bias, decision, grade, detail = scalp_confluence(
            sig1m, sig3m, sig5m, sig15m, news_boost
        )
        ...
        return (
            ...
            f"{vol_m}M",                    # 26 Vol24h
            round(sig5m["rel_vol"], 2),     # 27 RVol5M_tbl  (alias untuk kolom tabel)
            round(sig15m["rel_vol"], 2),    # 28 RVol15M_tbl
            sig1m["cond"],                  # 29 Cond1M
            sig5m["cond"],                  # 30 Cond5M
            sig15m["cond"],                 # 31 Cond15M
        )
