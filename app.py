...
# ======================================
# AUTO REFRESH (tiap 10 menit) — FULL SCAN
# ======================================
...
# ======================================
# FAST PRICE REFRESH (tiap 5 detik) — PRICE ONLY
# Update kolom Price/Chg24h/High/Low di _state["rows"] tanpa scan ulang indikator
# ======================================

def _price_refresh_loop():
    from scanner_scalp import refresh_price_cache, get_live_price
    while True:
        try:
            refresh_price_cache()
            with _lock:
                ...
                for r in _state["rows"]:
                    symbol = r[0]
                    price, change24h, high24h, low24h, vol24h, turnover24h = get_live_price(symbol)
                    if price > 0:
                        r = list(r)
                        r[22] = round(price, 6)
...
<div class="topbar">
  ...
  <span id="price-ts" class="ts" style="color:#00ff88">⚡ -</span>
</div>
...
<div class="table-wrap">
  <table id="main-table">
    ...
    <tbody id="table-body">
      <tr><td colspan="19" class="empty">Tekan Scan untuk mulai...</td></tr>
    </tbody>
  </table>
</div>
...
async function fetchStatus() {
  try {
    ...
    if (d.price_ts)
      document.getElementById('price-ts').textContent = '⚡ Price: ' + d.price_ts;

    // Rows (update terus, baik saat scanning maupun tidak, agar harga realtime tetap jalan)
...
function renderTable(rows) {
  ...
  if (!rows.length) {
    tbody.innerHTML = '<tr><td colspan="19" class="empty">Tidak ada coin yang memenuhi filter.</td></tr>';
    return;
  }
  ...
  tbody.innerHTML = rows.map((r, i) => {
    ...
    const vwapPos = String(r[36]);
    const vwapCls = vwapPos === 'Above' ? 'long' : 'short';
    const oiChg   = r[39];
    const oiCls   = oiChg > 0.5 ? 'long' : oiChg < -0.5 ? 'short' : 'flat';
    const src     = String(r[40]);
    const srcCls  = src === 'WS' ? 'long' : 'flat';
    ...
    return `<tr>
      ...
      <td class="${vwapCls}">${vwapPos}</td>
      <td class="${oiCls}">${oiChg > 0 ? '+' : ''}${oiChg}%</td>
      <td class="${rvCls}">${rv5}x</td>
      <td class="${r[10] >= 2 ? 'rvol-spike' : r[10] >= 1.5 ? 'rvol-high' : 'rvol-ok'}">${r[10]}x</td>
      <td>${r[7]}</td>
      <td>${r[22]}</td>
      <td class="${srcCls}" style="font-size:10px">${src}</td>
